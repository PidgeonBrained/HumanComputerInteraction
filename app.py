...
def display_disclaimer(window_surface):
    disclaimer_text = (
        f"Fitt's Law Project - CS 470 Spring 2024"
        #"By proceeding with this test, you acknowledge and agree to the terms and conditions.\n"
        #"Click the green box to confirm and start the test, or press 'ESC' to exit."
    )

    font = pygame.font.Font('freesansbold.ttf', 24)
    text = font.render(disclaimer_text, True, 'white', 'black')
    textRect = text.get_rect(center=(window_surface.get_width() // 2, window_surface.get_height() // 2))
    
    confirmation_button = pygame.Rect((window_surface.get_width() // 2 - 50, window_surface.get_height() // 2 + 50, 100, 50))
    ...
    while True:
        ...
        pygame.draw.rect(window_surface, 'green', confirmation_button)
        window_surface.blit(text, textRect)
        window_surface.blit(confirmation_text, confirmation_rect)
        pygame.display.flip()
